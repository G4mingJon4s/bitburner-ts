...
type __StringKeys<T, K> = `${K & string}` | ([T] extends [readonly (infer _)[]]
    ? number extends T["length"] ? (number extends K ? `${bigint}` : `${K & number}`) : never
...
export type AutoPath<O, P extends string> = (P & `${string}.` extends never ? P : P & `${string}.`) extends infer Q
? Q extends `${infer A}.${infer B}`
    ? (A extends StringKeys<O> ? `${A}.${AutoPath<GetStringKey<O, A>, B>}` : never)
    : Q extends StringKeys<O>
        ? Exclude<P, `${Q}.`> | (StringKeys<GetStringKey<O, Q>> extends never ? never : `${Q}.`)
        : StringKeys<O>
: never;

export type GetPath<O, P extends string> = P extends `${infer A}.${infer B}`
