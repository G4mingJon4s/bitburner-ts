import type { AutoPath } from "util/do";
...
export function execute<T>(ns: NS, options: ExecuteOptions, task: (ns: NS) => Promise<T>): Promise<T> {
    return new Promise((res, rej) => {
        ...
        const pid = ns.exec(FILEPATH, host, {
...
export const getRamCost = <T extends string>(ns: NS, functions: AutoPath<NS, T>[]): number => 1.6 + functions.reduce((acc, func) => acc + ns.getFunctionRamCost(func), 0);